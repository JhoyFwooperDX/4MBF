...
•   GITHUB : HTTPS://GITHUB.COM/AOREC XD/4MBF  •'''
	oi = nel(tekz(ban,justify='center',style='bold cyan'), style='bold')
...
def login_lagi334():
	try:
		...
		print('\nLOGIN BERHASIL  --->jalankan ulang ketik python 4mbf.py ');time.sleep(1)
		exit()
	except Exception as e:
...
def crack_file():
	...
	sol().print(mark(cek, style='green'))
	try:vin = os.listdir('/sdcard/NiammiftDita/')
	except FileNotFoundError:
		gada = '# STORAGE NOT FOUND '
		sol().print(mark(gada, style='red'))
		time.sleep(2)
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/NiammiftDita/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
			...
			sol().print(mark(ric, style='red'))
			exit()
		try:lin = open('/sdcard/NiammiftDita/'+geh,'r').read().splitlines()
		except:
			hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
			sol().print(mark(hehe, style='red'))
			time.sleep(2)
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/NiammiftDita/')
	except:pass
	login()
